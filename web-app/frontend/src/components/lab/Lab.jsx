...
import { get_lab, get_tasks_for_lab } from '../../api/api_lab.js';
import { get_sessions_for_lab } from '../../api/api_session.js';
import { toast, ToastContainer } from 'react-toastify';
import { AuthContext } from '../../contexts/AuthContext.jsx';
import Header from '../../components/Header.jsx';
import { FiPlus, FiHardDrive, FiFileText, FiUsers } from 'react-icons/fi';
    ...
    const fetchLab = async () => {
      try {
        ...
        const sortedSessions = sessionsData
          .sort((a, b) => parseDateTime(b.session_start) - parseDateTime(a.session_start));
            ...
            {pendingTasks.length > 0 ? (
              <ul className="flex flex-col gap-3 overflow-y-auto max-h-[300px]">
                {pendingTasks.map((task) => (
                  ...
                  >
                    <span className="font-medium text-gray-200 truncate" title={task.task_name}>
                      {task.task_name}
                    </span>
                  ...
                  sessions.map((session, idx) => (
                    <tr key={idx} className="even:bg-gray-800 odd:bg-gray-700">
                      ...
                      <td className="p-2 border border-gray-600">
                        {formatDateTime(parseDateTime(session.session_start))}
                      </td>
